...
RC INLJoin::getNextTuple(void *data)
{
  ...
  for (size_t i = 0; i < outerAttrs.size(); ++i) {
    char target = *((char*)outerRelation + i/8);
    if (target & (1<<(7-i%8))) continue;
    if (outerAttrs[i].type == TypeVarChar) {
        int size;
  ...
  for (size_t i = 0; i < innerAttrs.size(); ++i) {
    char target = *((char*)innerRelation + i/8);
    if (target & (1<<(7-i%8))) continue;
    if (innerAttrs[i].type == TypeVarChar) {
        int size;
